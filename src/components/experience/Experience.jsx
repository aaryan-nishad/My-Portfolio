    ...
    <section id='experience' className='relative overflow-hidden flex flex-col text-gray-600 body-font'>
        <div className='absolute top-[100px] left-64 inset-x-0 flex items-start justify-center'>
            ...
            <div className='container mx-auto flex px-5 py-24 md:flex-row flex-col items-center'>
                ...
                <img src={experience.image} alt="" className='object-cover md:w-[300px] md:h-[450px] object-center rounded relative z-10 '/>
                </div>
        ...
        {experience.entries.map((entry) => (
          <React.Fragment key={entry.id}>
            <h2 className='title-font font-bold text-white sm:text-2xl'>{entry.year}</h2>

            <p className='mb-8 text-white leading-relaxed'>{entry.description}</p>
          </React.Fragment>
        ))}
        </div>
        </div>
